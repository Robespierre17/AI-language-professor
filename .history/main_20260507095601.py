...
def chat(
    ...
    """Stream one user turn. Yields tagged tuples:
      ("token", chunk)    — every text delta from Claude, for live terminal printing
      ("sentence", sent)  — each complete sentence, for TTS handoff

    Tokens stream in real time; sentences fire when a boundary is found. The
    caller reads `session.target_language_code` after each yield to pick the
    right TTS voice — that value reflects the language the just-yielded sentence
    was generated in, since tool-use processing only happens after a stream's
    final flush.
    """
...
def main() -> None:
    ...
    try:
        while True:
            ...
            # TTS runs on a worker thread so audio playback for sentence N
            # doesn't block tokens for sentence N+1 from printing live.
            ...
            def tts_worker() -> None:
                while True:
                    ...
                    if item is None:
                        break
                    sentence, voice_id = item
                    try:
                        speak(eleven, sentence, voice_id)
                    except Exception as e:  # noqa: BLE001 — boundary call
                        print(f"\n⚠️  tts failed: {e}", file=sys.stderr)

            worker = threading.Thread(target=tts_worker, daemon=True)
            worker.start()

            lang = session.target_language_code
            print(f"tutor [{lang}]: ", end="", flush=True)
            for kind, payload in chat(claude, session, text):
                if kind == "token":
                    print(payload, end="", flush=True)
                else:  # "sentence"
                    lang = session.target_language_code
                    voice_id = VOICE_MAP.get(lang, DEFAULT_VOICE_ID)
                    tts_queue.put((payload, voice_id))
                    assistant_sentences.append(payload)
            print(flush=True)
